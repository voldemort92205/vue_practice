...
import CWADatasetRainfallLevelView from './CWADatasetRainfallLevelView.vue';
import CWADatasetWeatherStatusView from './CWADatasetWeatherStatusView.vue';
...
const rainfallStationDataset = reactive([]);
const rainfallStationRefreshTime = ref("Null");
const updateRainfallList = () => {
    const remoteData = cwaStore.dataSetRainfall[0].dataset;
    rainfallStationDataset.splice(0, rainfallStationDataset.length)
    remoteData.forEach((item) => {
        rainfallStationDataset.push(item);
    });
    rainfallStationRefreshTime.value = cwaStore.refreshTimeRainfall;
}

const weatherStatusDataset = reactive([]);
const weatherStationRefreshTime = ref("");
const updateWeatherMonitorList = () => {
    weatherStatusDataset.splice(0, weatherStatusDataset.length);
    cwaStore.dataSetWeatherMonitor.dataset.forEach((item) => {
        if (item.WeatherElement.Weather === "-99")
        {
            return;
        }
        weatherStatusDataset.push(item);
    })
    weatherStationRefreshTime.value = cwaStore.refreshTimeWeatherMonitor;
}
...
<template>
    <div class="relative">
        ...
        <CWADatasetRainfallLevelView :dataset="rainfallStationDataset" :refreshTime="rainfallStationRefreshTime" />
        <CWADatasetWeatherStatusView :dataset="weatherStatusDataset" :refreshTime="weatherStationRefreshTime" />
